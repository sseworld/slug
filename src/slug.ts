...
function getWholeCharAndI(str: string, i: number): [string, number] {
  ...
  if (code >= 0xD800 && code <= 0xDBFF) {
    if (str.length <= (i + 1)) {
      return [' ', i]; // High surrogate without following low surrogate
    }
  ...
  // Low surrogate
  if (i === 0) {
    return [' ', i]; // Low surrogate without preceding high surrogate
  }
...
function slugify(string: string, opts?: slug.Options): string {
